...
class DepthRules:
    ...
    @Rule(Architecture(family = 'mlp'),Problem(dataset_size='larg'))
    ...
    @Rule (Architecture(family ='cnn_scratch'),Problem(dataset_size='larg'))
    ...
    def cnn_pretrained (self):
        self.declare(Blueprint(hidden_layers='fixed by pretrained backbone',note='only classification depth is user_set(1-2FC layers)'))
        
    #____________________________________
    #conv_autocoder
    #Gong et al. 2019 · arxiv:1904.11294
    @Rule(Architecture(family='conv_autoencoder'),Problem(dataset_size=L('tiny')|L('small')))
    ...
    @Rule(Architecture(family='conv_autoencoder'),Problem(dataset_size=L('medium')|L('larg')),salience=10)
    def conv_autoencoder_depth_larg (self):
        self.declare(Blueprint(hiddenlayers='encoder 4 to 5 conv blocks -> bottelneck _. decoder:mirror'))
    ...
    @Rule (Architecture(family='lstm'),Problem(dataset_size ='larg'),salience=10)
    ...
    @Rule (Architecture('autoencoder'),salience=10)
    ...
    @Rule(Architecture(family='lstm_autoencoder'),Problem(dataset_size=L('medium')|L('larg')),salience=10)
    ...
    def bert_depth(self):
        self.declare(Blueprint(hidden_layer='12 transformer layers (bert base) or 24 (bert base) fixed by pretrained model'))
    ...
    @Rule(Architecture(family='transformer_ts'),Problem(dataset_size=L('medium')|L('larg')),salience=10)
