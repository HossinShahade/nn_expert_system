...
class InputLayerRules:
    ...
    def mlp_input_large(self):
        self.declare(Blueprint(
            input_layer='linear(fetures->512)',
            notes='consider PCA to reduce diminsionality first'
        ))
    ...
    def cnn_scratch_input_small(self):
        self.declare(Blueprint(input_layer='conv2D(16,3X3,padding=1)->BN->ReLU, no early pooling'))

    @Rule(Architecture(family='cnn_scratch'), Problem(input_shape='medium'), salience=10)
    def cnn_scratch_medium_input(self):
        self.declare(Blueprint(input_layer='conv2D(32,3X3,stride=1)->BN->ReLU->MAXPool(2X2)'))

    @Rule(Architecture(family='cnn_scratch'), Problem(input_shape='large'), salience=10)
    def cnn_scratch_large_input(self):
    ...
    def cnn_pretrained_input(self):
        self.declare(Blueprint(input_layer='resize to 224X224, normalize with imagenet mean=[0.485,0.456,0.406] std=[0.229,0.224,0.225]'))
    ...
    def lstm_input_timeseries(self):
        self.declare(Blueprint(input_layer='RAW features per timestep no embedding needed'))
    ...
    def input_bert(self):
        self.declare(Blueprint(input_layer='Tokenizer->token_ids + attention_mask, max_length=512'))
    ...
    def gpt_input(self):
        self.declare(Blueprint(input_layer='Tokenizer->token_ids, max length varies by model variant'))
    ...
    def transformer_ts_input(self):
        self.declare(Blueprint(input_layer='linear projection of input features to model_dim, add positional encoding'))
    ...
    def xgboost_input(self):
        self.declare(Blueprint(
            input_layer='RAW feature vector, no layer needed',
            notes='apply StandardScaler before fitting'
        ))
    ...
    def logreg_input(self):
        self.declare(Blueprint(
            input_layer='RAW feature vector',
            notes='apply StandardScaler -- logistic regression is sensitive to scale'
        ))
    ...
    def yolo_input(self):
        self.declare(Blueprint(input_layer='resize to 640X640, normalize pixel values to [0,1]'))
    ...
    def gan_input(self):
        self.declare(Blueprint(input_layer='Generator: random noise vector z ~ N(0,1), dim=100 -- Discriminator: real or generated sample'))